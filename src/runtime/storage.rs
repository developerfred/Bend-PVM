...
impl StorageManager {
    ...
    /// Set a storage value
    pub fn set(&mut self, key: &[u8], value: &[u8], metering: &mut MeteringContext) -> Result<(), StorageError> {
        // Validate key and value sizes
        if key.len() > self.limits.max_key_size {
            return Err(StorageError::KeyTooLarge(key.len(), self.limits.max_key_size));
        }
        
        if value.len() > self.limits.max_value_size {
            return Err(StorageError::ValueTooLarge(value.len(), self.limits.max_value_size));
        }
        
        // Check storage limit
        if !self.storage.contains_key(&self.namespaced_key(key)) && self.storage.len() >= self.limits.max_storage_items {
            return Err(StorageError::StorageLimitExceeded);
        }
        
        // Charge for storage operation
        metering.charge_storage_write(key, value)
    ...
    /// Get a storage value
    pub fn get(&mut self, key: &[u8], metering: &mut MeteringContext) -> Result<Option<Vec<u8>>, StorageError> {
        // Validate key size
        if key.len() > self.limits.max_key_size {
            return Err(StorageError::KeyTooLarge(key.len(), self.limits.max_key_size));
        }
        ...
        // Charge for storage operation
        metering.charge_storage_read(key)
            .map_err(|e| StorageError::Metering(e.to_string()))?;
    ...
    pub fn contains(&mut self, key: &[u8], metering: &mut MeteringContext) -> Result<bool, StorageError> {
        // Validate key size
        if key.len() > self.limits.max_key_size {
            return Err(StorageError::KeyTooLarge(key.len(), self.limits.max_key_size));
        }
    ...
    pub fn remove(&mut self, key: &[u8], metering: &mut MeteringContext) -> Result<(), StorageError> {
        // Validate key size
        if key.len() > self.limits.max_key_size {
            return Err(StorageError::KeyTooLarge(key.len(), self.limits.max_key_size));
        }
        ...
        // Charge for storage operation
        metering.charge_storage_delete(key)
            .map_err(|e| StorageError::Metering(e.to_string()))?;
...
impl ChildStorage {
    ...
    pub fn keys(&self) -> Vec<Vec<u8>> {
        self.parent.prefix_iter(&self.prefix)
            .into_iter()
