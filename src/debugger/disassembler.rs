...
impl Disassembler {
    ...
    pub fn disassemble_function(&self, function_name: &str) -> Option<Vec<DisassembledInstruction>> {
    ...
    pub fn disassemble_context(&self, pc: usize, context_count: usize) -> Vec<DisassembledInstruction> {
...
impl DisassembledInstruction {
    ...
    pub fn to_string_with_source(&self) -> String {
        if let Some(line) = self.source_line {
            format!("{:08x}: {} // line {}", self.address, self.instruction, line)
        } else {
            format!("{:08x}: {}", self.address, self.instruction)
        }
    }
