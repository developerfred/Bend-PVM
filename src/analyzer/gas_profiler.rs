...
impl GasProfiler {
    ...
    pub fn profile_source(&self, source: &str, file_path: &str) -> Result<GasProfile, ProfilerError> {
        // Parse the source
        ...
        let most_expensive_function = estimates.iter()
    ...
    fn profile_statement(&self, statement: &Statement, cost_breakdown: &mut HashMap<String, u64>) -> u64 {
        match statement {
            ...
            Statement::Bend { condition, initial_states, body, else_body, .. } => {
                ...
                *cost_breakdown.entry("bend_iteration".to_string()).or_insert(0) += bend_cost * estimated_iterations;
                
                bend_cost * estimated_iterations + condition_cost + initial_states_cost + (body_cost * estimated_iterations) + else_cost
            }
            Statement::Match { value, cases, .. } => {
                ...
                *cost_breakdown.entry("match_branch".to_string()).or_insert(0) += match_cost;
            ...
            Statement::With { monad_type, body, .. } => {
                ...
                *cost_breakdown.entry("with_block".to_string()).or_insert(0) += with_cost;
                if external_cost > 0 {
                    *cost_breakdown.entry("io_operations".to_string()).or_insert(0) += external_cost;
                }
                
                with_cost + body_cost + external_cost
            }
            Statement::Expr { expr, .. } => {
                self.profile_expr(expr, cost_breakdown)
            }
    ...
    fn profile_expr(&self, expr: &Expr, cost_breakdown: &mut HashMap<String, u64>) -> u64 {
        match expr {
            Expr::Variable { .. } => {
                ...
                *cost_breakdown.entry("variable_access".to_string()).or_insert(0) += cost;
                cost
            }
            Expr::Literal { .. } => {
            ...
            Expr::FunctionCall { function, args, .. } => {
                ...
                if special_cost > 0 {
                    ...
                    *cost_breakdown.entry("function_call".to_string()).or_insert(0) += call_cost;
                }
            ...
            match statement {
                ...
                Statement::If { condition, then_branch, else_branch, .. } => {
                    if self.expr_calls_function(condition, function_name) ||
                       ...
                       (else_branch.as_ref().map_or(false, |b| self.contains_call_to(b, function_name))) {
                        return true;
                    }
                }
                Statement::Match { value, cases, .. } => {
                ...
                Statement::Bend { condition, initial_states, body, else_body, .. } => {
                    ...
                    if else_body.as_ref().map_or(false, |b| self.contains_call_to(b, function_name)) {
                        return true;
                    }
                }
                Statement::With { body, .. } => {
    ...
    fn expr_calls_function(&self, expr: &Expr, function_name: &str) -> bool {
        match expr {
            ...
            Expr::BinaryOp { left, right, .. } => {
                self.expr_calls_function(left, function_name) || self.expr_calls_function(right, function_name)
            }
            Expr::Tuple { elements, .. } => {
                elements.iter().any(|e| self.expr_calls_function(e, function_name))
            }
            Expr::List { elements, .. } => {
                elements.iter().any(|e| self.expr_calls_function(e, function_name))
            }
            Expr::Constructor { args, .. } => {
                args.iter().any(|e| self.expr_calls_function(e, function_name))
            }
            ...
            match statement {
                ...
                Statement::If { condition, then_branch, else_branch, .. } => {
                    if self.expr_has_external_call(condition) ||
                       ...
                       (else_branch.as_ref().map_or(false, |b| self.has_external_calls(b))) {
                        return true;
                    }
                }
                Statement::Match { value, cases, .. } => {
                ...
                Statement::Bend { condition, initial_states, body, else_body, .. } => {
                    ...
                    if else_body.as_ref().map_or(false, |b| self.has_external_calls(b)) {
                        return true;
                    }
                }
                Statement::With { body, .. } => {
        ...
        match expr {
            Expr::FunctionCall { function, args, .. } => {
                ...
                let is_external_call = if let Expr::Variable { name, .. } = &**function {
                    name.starts_with("IO/call") || name.starts_with("IO/static_call") ||
                    name.starts_with("IO/delegatecall") || name.starts_with("transfer")
                } else {
                    false
                };
            ...
            Expr::Tuple { elements, .. } => {
                elements.iter().any(|e| self.expr_has_external_call(e))
            }
            Expr::List { elements, .. } => {
                elements.iter().any(|e| self.expr_has_external_call(e))
            }
            Expr::Constructor { args, .. } => {
                args.iter().any(|e| self.expr_has_external_call(e))
            }
            // Check other expression types
            _ => false,
        }
    ...
    for statement in &block.statements {
        line_count += match statement {
            Statement::If { then_branch, else_branch, .. } => {
                2 + count_lines(then_branch) + else_branch.as_ref().map_or(0, count_lines)
            }
            Statement::Match { cases, .. } => {
                1 + cases.iter().map(|case| 1 + count_lines(&case.body)).sum::<usize>()
            }
            Statement::Bend { body, else_body, .. } => {
                2 + count_lines(body) + else_body.as_ref().map_or(0, count_lines)
            }
            Statement::With { body, .. } => {
                1 + count_lines(body)
            }
...
pub fn print_profile(profile: &GasProfile) {
    ...
    if profile.estimates.is_empty() {
        ...
        for (i, estimate) in profile.estimates.iter().enumerate() {
            println!("{}. {} (lines {}-{})", i + 1, estimate.name, estimate.line_range.0, estimate.line_range.1);
