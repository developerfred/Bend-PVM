...
impl FloatCombPass {
    ...
    fn optimize_expr(&self, expr: &Expr) -> (Expr, bool) {
        match expr {
            Expr::BinaryOp { left, operator, right, location } => {
                ...
                if let Some(folded) = self.fold_constants(&optimized_left, operator, &optimized_right, location) {
                    return (folded, true);
                }
                
                // Try algebraic simplifications
                if let Some(simplified) = self.simplify_algebra(&optimized_left, operator, &optimized_right, location) {
    ...
    fn fold_constants(&self, left: &Expr, operator: &BinaryOperator, right: &Expr, location: &Location) -> Option<Expr> {
        match (left, operator, right) {
            // Float + Float -> Float
            (
                Expr::Literal { kind: LiteralKind::Float(left_val), .. },
                BinaryOperator::Add,
                Expr::Literal { kind: LiteralKind::Float(right_val), .. }
            ) => {
                Some(Expr::Literal {
    ...
    fn simplify_algebra(&self, left: &Expr, operator: &BinaryOperator, right: &Expr, location: &Location) -> Option<Expr> {
        match (left, operator, right) {
            // x + 0 -> x
            (
                expr,
                BinaryOperator::Add,
                Expr::Literal { kind: LiteralKind::Float(val), .. }
            ) if *val == 0.0 => {
                Some(expr.clone())
            },
            // 0 + x -> x
            ...
            (
                _,
                BinaryOperator::Mul,
                Expr::Literal { kind: LiteralKind::Float(val), location: right_loc, .. }
            ) if *val == 0.0 => {
                Some(Expr::Literal {
            ...
            // 0 * x -> 0
            (
                Expr::Literal { kind: LiteralKind::Float(val), location: left_loc, .. },
                BinaryOperator::Mul,
                _
            ) if *val == 0.0 => {
                Some(Expr::Literal {
