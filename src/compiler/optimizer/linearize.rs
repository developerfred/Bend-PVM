    ...
    fn linearize_statement(&mut self, statement: &Statement) -> (Vec<Statement>, bool) {
        match statement {
            ...
            Statement::If { condition, then_branch, else_branch, location } => {
                ...
                let (linearized_condition, mut pre_statements, condition_modified) = self.linearize_expr(condition);
            ...
            Statement::Expr { expr, location } => {
                let (linearized_expr, mut pre_statements, expr_modified) = self.linearize_expr(expr);
    ...
    fn linearize_expr(&mut self, expr: &Expr) -> (Expr, Vec<Statement>, bool) {
        match expr {
            Expr::BinaryOp { left, operator, right, location } => {
                ...
                let (linearized_left, mut left_statements, left_modified) = self.linearize_expr(left);
                let (linearized_right, mut right_statements, right_modified) = self.linearize_expr(right);
                ...
                let (final_right, mut right_temp_stmts) = self.maybe_extract_expr(&linearized_right);
                ...
                let modified = left_modified || right_modified || 
                    final_left != linearized_left || final_right != linearized_right;
            ...
            Expr::FunctionCall { function, args, named_args, location } => {
                ...
                let (linearized_function, mut function_statements, function_modified) = self.linearize_expr(function);
                ...
                for arg in args {
                    let (linearized_arg, mut arg_statements, arg_modified) = self.linearize_expr(arg);
                ...
                let (final_function, mut function_temp_stmts) = self.maybe_extract_expr(&linearized_function);
